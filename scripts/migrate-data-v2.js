...
import { hdImageData } from '../src/data/original-data.js';
import fs from 'fs';
...
try {
  ...
  const convertedData = hdImageData.map((item, index) => {
    ...
    if (title.includes('bunny')) tags.push('兔子');
    if (title.includes('doll')) tags.push('玩偶');
...
// 导出原始数据（用于调试）
export const rawData = ${JSON.stringify(hdImageData.slice(0, 5), null, 2)};`;
